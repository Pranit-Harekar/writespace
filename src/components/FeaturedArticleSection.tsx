  ...
  useEffect(() => {
    const fetchFeaturedArticle = async () => {
      try {
        ...
        const { data: featuredArticleData, error: featuredError } =
          await supabase
            .from('articles')
            .select(
              `
          ...
          if (authorError) {
            ...
            const formattedArticle = {
              ...
              author: {
                id: authorData.id,
                name:
                  authorData.full_name || authorData.username || 'Anonymous',
        ...
        if (error.code === 'PGRST116') {
          toast({
            title: 'Oops!',
            description:
              'No published articles found. Please check back later!',
            variant: 'default',
          })
        } else {
          toast({
